    ...
    <div class="chat-container bg-surface-2">
        <div v-if="uploadedImageUrl" class="image-preview flex">
            <div class="preview-holder centered">
                ...
                <button v-if="!uploading" @click="deleteImage(uploadedImageUrl)" class="image-uploader absolute jc-c ai-c" :class="!(uploading || deleting) ? 'image-uploader-idle' : 'image-uploader-processing'">
        ...
        <form @submit.prevent="" action="" class="flex ai-fe">
            <textarea v-model="prompt" class="flex" rows="1" @input="adjustHeight" @keydown.enter="submitPrompt" ref="textArea" name="prompt" id="prompt_input" placeholder="Enter a prompt here"></textarea>
