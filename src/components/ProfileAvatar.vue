<template>
    <div class="relative centered avatar-wrapper">
        <img v-if="picture" class="avatar br-50" :src="picture" :alt="computedInitial">
        <div v-else class="centered avatar avatar-font br-50"><strong>{{ computedInitial }}</strong></div>
        <button @click.prevent="handleButtonClick" v-if="upload" class="absolute pd-0 centered" :disabled="deleting || uploading">
            <svg v-if="!picture || uploading" height="14" viewBox="0 0 36 30">
            ...
            <svg v-if="picture && !uploading" height="16" viewBox="0 0 15.3 17">
        ...
        <div v-if="upload && (uploading || deleting)" class="img-overlay centered absolute br-50">
            <spinner :size="28" />
        </div>
    </div>
</template>
    ...
    methods: {
        handleButtonClick() {
            if (!this.picture) {
                ...
                this.$emit('delete-image', this.picture)
            }
        }
    }
}
...
.avatar-wrapper {
    height: v-bind(computedDimension);
    width: v-bind(computedDimension);
    z-index: 1;
}
.avatar {
    height: inherit;
