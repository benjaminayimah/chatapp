<template>
    <li class="flex" @mouseup="device === 'mobile' ? $store.commit('closeSideBar') : ''">
        <router-link  :to="!editMode ? { name: 'ChatView', params: { id: recent.id }} : ''" data-type="menu" class="flex ai-c" :class="{'is-toggled' : dropdownToggle}">
            ...
            <input @keydown.enter="handleKeyDownEnter" @focusout="handleFocusOut"  ref="input" v-model="input" v-if="editMode" type="text" name="renameChat" class="bg-transparent">
    ...
    <teleport to="body">
        <backdrop @click="handleDropdown" v-if="dropdownToggle" :opacity="0" :zindex="101" />
    ...
    methods: {
        handleDropdown() {
            if (!this.dropdownToggle) {
                ...
                this.$store.commit('setDropdown', `recent_${this.recent.id}`)
            } else {
